...
namespace
{
    ...
    enum DisplayOption : uint8_t
    {
        DisplayDesignOption,
        DisplayTimeoutEnabledOption,
        DisplayTimeoutValueOption,
    ...
    enum PerifericsOption : uint8_t
    {
        PerifericsKeypad,
        PerifericsBack,
        PerifericsOptionCount,
    };

    enum KeypadOption : uint8_t
    {
        KeypadInfoOption,
        KeypadTimeoutOption,
        KeypadEventStateOption,
        KeypadEventValueOption,
        KeypadResetOption,
        KeypadBack,
        KeypadOptionCount,
    };
    ...
    const char *const displayOptions[DisplayOptionCount] = {
        "Design",
        "Timeout enabled",
        "Timeout value",
        "Active mode",
    ...
    const char *const homeDesignNames[DISPLAY_HOME_DESIGN_COUNT_DEFAULT] = {
        "Classic",
        "Compact",
        "Data",
    };

    const char *const displayActiveModeNames[] = {
        "AllTime",
        "ClockTime",
        "UsbMounted",
    };

    const char *const perifericsOptions[PerifericsOptionCount] = {
        "Keypad",
        "Back",
    };

    const char *const keypadOptions[KeypadOptionCount] = {
        "Info",
        "Timeout",
        "Event state",
        "Event value",
    ...
    const char *const busUnitOptions[BusUnitOptionCount] = {
        "Celsius/Fahrenheit",
        "ppm/%",
        "Back",
    };
    ...
    uint8_t selectedBusUnit = BusUnitTemperature;
    ...
    uint32_t periodicRedrawInterval()
    {
        switch (activeScreen)
        {
            ...
            case Screen::KeypadEventState:
            case Screen::KeypadEventValue:
            case Screen::SystemDisplayMenu:
            case Screen::BusMenu:
            case Screen::BusScan:
            case Screen::BusBasics:
            case Screen::BusSensorsMenu:
            case Screen::BusDeviceStatus:
            case Screen::LightPowerMenu:
    ...
    const char *currentMenuPath()
    {
        switch (activeScreen)
        {
            ...
            case Screen::LightKelvinHotMax:
                return "menu/light/kelvin";
            case Screen::LightPowerMenu:
                return "menu/light/power";
            ...
            case Screen::LightKelvinStep:
                return "menu/buttons";
            case Screen::BusMenu:
                return "menu/system/bus";
            case Screen::BusScan:
                return "menu/system/bus/scan";
            case Screen::BusBasics:
                return "menu/system/bus/basics";
            case Screen::BusSensorsMenu:
            case Screen::BusDeviceStatus:
                return "menu/system/bus/sensors";
            case Screen::SystemSensorsMenu:
            case Screen::BusUnitsMenu:
            case Screen::BusGasUnit:
            case Screen::SensorSampleInterval:
            case Screen::SensorSampleCount:
            case Screen::TemperatureUnit:
                return "menu/system/bus/settings";
            case Screen::RtcMenu:
                return "menu/rtc";
            case Screen::RtcSetTime:
                return "menu/rtc/time";
            case Screen::RtcSetDate:
                return "menu/rtc/date";
            ...
            case Screen::RtcAlarmDelete:
                return "menu/rtc/alarms";
            case Screen::SystemMenu:
                return "menu/system";
            case Screen::SystemDisplayMenu:
            case Screen::DisplayDesign:
            case Screen::DisplayTimeoutEnabled:
            case Screen::DisplayActiveMode:
            ...
            case Screen::KeypadInfo:
            case Screen::KeypadTimeout:
            case Screen::KeypadEventState:
            case Screen::KeypadEventValue:
            case Screen::KeypadReset:
                return "menu/system/periferics/keypad";
            case Screen::ChipInfo:
                return "menu/system/chip";
    ...
    {
        Oled.setFont(OledFont::Small);
        const char *path = currentMenuPath();
        Oled.drawStr(0, 7, path ? path : title);
        drawTaskbarIcons();
    ...
    const char *displayActiveModeName(uint8_t mode)
    {
        if (mode > DISPLAY_ACTIVE_MODE_USB_MOUNTED_DEFAULT)
        {
            mode = DISPLAY_ACTIVE_MODE_DEFAULT;
        }

        return displayActiveModeNames[mode];
    }

    void formatMinutesOfDay(uint16_t minutesOfDay, char *buffer, size_t length)
    {
        minutesOfDay = constrain(minutesOfDay, 0, 1439);
        snprintf(buffer, length, "%02u:%02u", minutesOfDay / 60, minutesOfDay % 60);
    }

    bool clockWindowActive(uint16_t nowMinute, uint16_t startMinute, uint16_t endMinute)
    {
    ...
    bool displayShouldBeActive()
    {
        const DeskSettings::Config &settings = DeskSettings::dataConst();

        switch (settings.displayActiveMode)
        {
            case DISPLAY_ACTIVE_MODE_CLOCK_TIME_DEFAULT:
            {
                ...
                const uint16_t nowMinute = static_cast<uint16_t>(now.hour() * 60 + now.minute());
                return clockWindowActive(nowMinute, settings.displayActiveStartMinute, settings.displayActiveEndMinute);
            }
            case DISPLAY_ACTIVE_MODE_USB_MOUNTED_DEFAULT:
                return DeskUSB::mounted();
            case DISPLAY_ACTIVE_MODE_ALL_TIME_DEFAULT:
            default:
                return true;
        }
    }
    ...
    void drawHomeClassic(const char *boxBigText, const char *boxSmallText, const char *boxData1Text, const char *boxData2Text, const char *boxData3Text)
    {
    ...
    void drawHomeCompact(const char *boxBigText, const char *boxSmallText, const char *boxData1Text, const char *boxData2Text, const char *boxData3Text)
    {
        Oled.drawHLine(0, 8, 128);
        Oled.setFont(OledFont::Text);
        Oled.drawStr(0, 7, "Home");
        drawTaskbarIcons();

        Oled.setFont(OledFont::HomeTime);
        Oled.drawStr(0, 32, boxBigText);
        Oled.setFont(OledFont::Small);
        Oled.drawStr(0, 44, boxSmallText);
        Oled.drawStr(72, 22, boxData1Text);
        Oled.drawStr(72, 38, boxData2Text);
        Oled.drawStr(72, 56, boxData3Text);
    }

    void drawHomeData(const char *boxBigText, const char *boxSmallText, const char *boxData1Text, const char *boxData2Text, const char *boxData3Text)
    {
        Oled.drawHLine(0, 8, 128);
        Oled.setFont(OledFont::Text);
        Oled.drawStr(0, 7, boxBigText);
        Oled.drawStr(70, 7, boxSmallText);
        drawTaskbarIcons();

        Oled.setFont(OledFont::Text);
        Oled.drawStr(0, 24, boxData1Text);
        Oled.drawStr(0, 40, boxData2Text);
        Oled.drawStr(0, 58, boxData3Text);
    }
    ...
    void drawHome()
    {
        ...
        Oled.clearBuffer();
        switch (DeskSettings::dataConst().displayDesign)
        {
            case 1:
                drawHomeCompact(boxBigText, boxSmallText, boxData1Text, boxData2Text, boxData3Text);
                break;
            case 2:
                drawHomeData(boxBigText, boxSmallText, boxData1Text, boxData2Text, boxData3Text);
                break;
            default:
                drawHomeClassic(boxBigText, boxSmallText, boxData1Text, boxData2Text, boxData3Text);
                break;
        }
        Oled.sendBuffer();
    }
    ...
    void drawUsbInfo()
    {
        ...
        Oled.print("  CDC ");
        Oled.print(DeskUSB::cdcConnected(DeskUSB::CONTROL) ? "OPEN" : "----");
        Oled.setCursor(0, 41);
        Oled.print("VBUS ");
        Oled.print(DeskUSB::vbusPresent() ? "YES" : "NO ");
        Oled.print("  ID ");
        Oled.print(DeskUSB::idGrounded() ? "GND" : "FLOAT");
        Oled.setCursor(0, 52);
        Oled.print("Product: DeskBridge");
        Oled.setCursor(0, 63);
        Oled.print("ESC: back");
        Oled.sendBuffer();
    }
    ...
    void drawKeypadInfo()
    {
        ...
        Oled.clearBuffer();
        drawTitle("Keypad info");
        Oled.setFont(OledFont::Small);
        Oled.setCursor(0, 20);
        Oled.print("State: ");
        Oled.print(keypad.present ? "OK" : "MISS");
        Oled.setCursor(0, 31);
        Oled.print("Protocol: ");
        Oled.print(keypad.protocolText[0] != '\0' ? keypad.protocolText : "TXT?");
        Oled.setCursor(0, 42);
        Oled.print("Buttons: UART local");
        Oled.setCursor(0, 53);
        Oled.print("Event pin: ");
        Oled.print(keypad.eventInterruptEnabled ? "IRQ ON" : "OFF");
        Oled.setCursor(0, 63);
        Oled.print("ESC: back");
        Oled.sendBuffer();
    }
    ...
    void drawKeypadEventState()
    {
        const KeypadPeripheral::Snapshot &keypad = KeypadPeripheral::snapshot();

        Oled.clearBuffer();
        drawTitle("Event state");
        Oled.setFont(OledFont::Value);
        Oled.setCursor(8, 38);
    ...
    void drawKeypadEventValue()
    {
        const KeypadPeripheral::Snapshot &keypad = KeypadPeripheral::snapshot();

        Oled.clearBuffer();
        drawTitle("Event value");
        Oled.setFont(OledFont::Small);
        Oled.setCursor(0, 20);
        Oled.print("EVT:");
        Oled.print(keypad.events);
        Oled.print(" IRQ:");
        Oled.print(keypad.eventInterruptCount);
        Oled.setCursor(0, 32);
        Oled.print("BP:");
        Oled.print(keypad.buttonPressedMask);
        Oled.print(" BR:");
        Oled.print(keypad.buttonReleasedMask);
        Oled.print(" BD:");
        Oled.print(keypad.buttonDownMask);
        Oled.setCursor(0, 44);
        Oled.print("LB:");
        Oled.print(keypad.lastButton);
        Oled.print(" BE:");
        Oled.print(keypad.lastButtonEdge);
        Oled.setCursor(0, 56);
        Oled.print("ACT:");
        Oled.print(keypad.lastAction[0] != '\0' ? keypad.lastAction : "NONE");
        Oled.sendBuffer();
    }

    void drawKeypadReset()
    {
        char elapsedText[18];
        const uint32_t elapsedMs = millis() - lastKeypadResetMs;
        snprintf(elapsedText, sizeof(elapsedText), "Elapsed %lums", elapsedMs);

        Oled.clearBuffer();
        drawTitle("Keypad reset");
        Oled.setCursor(0, 24);
    ...
    void drawLightPower()
    {
        ...
        const KeypadPeripheral::Snapshot &keypad = KeypadPeripheral::snapshot();
        const uint32_t powerMw = static_cast<uint32_t>(abs(keypad.currentMa)) * keypad.supplyMv / 1000UL;

        Oled.clearBuffer();
        drawTitle("Light power");
        Oled.setFont(OledFont::Small);
        ...
        Oled.print("ADC ");
        Oled.print(keypad.currentRaw);

        Oled.setCursor(0, 31);
        Oled.print("V: ");
        Oled.print(keypad.supplyMv / 1000);
        Oled.print(".");
        Oled.print((keypad.supplyMv % 1000) / 100);
        Oled.print("V");

        Oled.setCursor(64, 31);
        Oled.print("I: ");
        Oled.print(keypad.currentMa);
        Oled.print("mA");

        Oled.setCursor(0, 42);
        Oled.print("P: ");
        Oled.print(powerMw / 1000);
        Oled.print(".");
        Oled.print((powerMw % 1000) / 100);
        Oled.print("W");

        Oled.setCursor(0, 53);
        Oled.print("C:");
        Oled.print(keypad.pwmCold);
        Oled.print(" W:");
        Oled.print(keypad.pwmWarm);
    ...
    {
        Oled.clearBuffer();
        drawTitle("Bus basics");
        Oled.setFont(OledFont::Small);
        Oled.setCursor(0, 20);
        Oled.print("DS3231  ");
        Oled.print(DeskBus::ready(DeskBus::Device::DS3231) ? "OK " : "-- ");
        Oled.print(DeskBus::addressText(DeskBus::Device::DS3231));
        Oled.setCursor(0, 34);
        Oled.print("AT24C32 ");
    ...
    void drawDisplayMenu()
    {
        ...
        uint8_t first = 0;
        if (selectedDisplay >= DISPLAY_MENU_VISIBLE_ROWS_DEFAULT)
        {
            first = selectedDisplay - (DISPLAY_MENU_VISIBLE_ROWS_DEFAULT - 1);
        }

        const DeskSettings::Config &settings = DeskSettings::dataConst();
        char value[12] = {};

        for (uint8_t row = 0; row < DISPLAY_MENU_VISIBLE_ROWS_DEFAULT && first + row < DisplayOptionCount; ++row)
        {
            const uint8_t option = first + row;
            const int y = 20 + row * 9;
            ...
            Oled.drawStr(4, y, displayOptions[option]);
            switch (option)
            {
                case DisplayDesignOption:
                    Oled.drawStr(88, y, homeDesignNames[DeskSettings::dataConst().displayDesign]);
                    break;
                case DisplayTimeoutEnabledOption:
                    Oled.drawStr(88, y, settings.displayTimeoutEnabled ? "On" : "Off");
                    break;
                case DisplayTimeoutValueOption:
                    snprintf(value, sizeof(value), "%lus", static_cast<unsigned long>(settings.displayTimeoutMs / 1000));
                    Oled.drawStr(88, y, value);
                    break;
                case DisplayActiveModeOption:
                    Oled.drawStr(76, y, displayActiveModeName(settings.displayActiveMode));
                    break;
                case DisplayActiveStartOption:
                    formatMinutesOfDay(settings.displayActiveStartMinute, value, sizeof(value));
                    Oled.drawStr(88, y, value);
                    break;
    ...
    void drawDisplayDesign()
    {
        Oled.clearBuffer();
        drawTitle("Home design");
        Oled.setFont(OledFont::Value);
        Oled.setCursor(8, 42);
        Oled.print(homeDesignNames[DeskSettings::dataConst().displayDesign]);
        Oled.setFont(OledFont::Small);
        Oled.drawStr(0, 62, "click/turn: change");
        Oled.sendBuffer();
    }

    void drawDisplayTimeoutEnabled()
    {
        Oled.clearBuffer();
        drawTitle("Timeout enabled");
        Oled.setFont(OledFont::Value);
        Oled.setCursor(18, 42);
        Oled.print(DeskSettings::dataConst().displayTimeoutEnabled ? "ON" : "OFF");
        Oled.setFont(OledFont::Small);
        Oled.drawStr(0, 62, "click/turn: toggle");
        Oled.sendBuffer();
    }

    void drawDisplayActiveMode()
    {
        Oled.clearBuffer();
        drawTitle("Active mode");
        Oled.setFont(OledFont::Text);
        Oled.setCursor(8, 35);
        Oled.print(displayActiveModeName(DeskSettings::dataConst().displayActiveMode));
        Oled.setFont(OledFont::Small);
        Oled.drawStr(0, 62, "click/turn: change");
        Oled.sendBuffer();
    }

    void drawDisplayActiveTime(const char *title, uint16_t minutesOfDay)
    {
        char text[8] = {};
        formatMinutesOfDay(minutesOfDay, text, sizeof(text));
        ...
        Oled.print(text);
        Oled.setFont(OledFont::Small);
        Oled.drawStr(0, 62, "turn: 15 min step");
        Oled.sendBuffer();
    }
    ...
    void drawSensorStatus()
    {
        ...
        Oled.setCursor(0, 30);
        Oled.print("KEYPAD ");
        Oled.print(DeskBus::ready(DeskBus::Device::Keypad) ? "OK" : "--");
    ...
    void goBack()
    {
        switch (activeScreen)
        {
            ...
            case Screen::KeypadEventValue:
            case Screen::KeypadReset:
                activeScreen = Screen::KeypadMenu;
                break;
            case Screen::SystemNotificationList:
            case Screen::SystemNotificationMenu:
    ...
    void openSelected()
    {
        switch (activeScreen)
        {
            ...
            case Screen::LightMenu:
                ...
                else if (selectedLight == LightPowerOption)
                {
                    KeypadPeripheral::update();
                    activeScreen = Screen::LightPowerMenu;
                }
                else
                {
                    activeScreen = Screen::LightButtonsMenu;
            ...
            case Screen::BusMenu:
                ...
                else if (selectedBus == BusBasicsOption)
                {
                    activeScreen = Screen::BusBasics;
                }
            ...
            case Screen::SystemMenu:
                ...
                else if (selectedSystem == SystemBus)
                {
                    activeScreen = Screen::BusMenu;
                }
                else if (selectedSystem == SystemUsb)
                {
                ...
                break;
            case Screen::SystemDisplayMenu:
                if (selectedDisplay == DisplayBack)
                {
                    activeScreen = Screen::SystemMenu;
                }
                else if (selectedDisplay == DisplayDesignOption)
                {
                    activeScreen = Screen::DisplayDesign;
                }
                else if (selectedDisplay == DisplayTimeoutEnabledOption)
                {
                    activeScreen = Screen::DisplayTimeoutEnabled;
                }
                else if (selectedDisplay == DisplayTimeoutValueOption)
                {
                    activeScreen = Screen::DisplayTimeout;
                }
                else if (selectedDisplay == DisplayActiveModeOption)
                {
                    activeScreen = Screen::DisplayActiveMode;
                }
                else if (selectedDisplay == DisplayActiveStartOption)
                {
                ...
                break;
            case Screen::SystemPerifericsMenu:
                activeScreen = selectedPeriferics == PerifericsBack ? Screen::SystemMenu : Screen::KeypadMenu;
                break;
            case Screen::KeypadMenu:
                if (selectedKeypad == KeypadBack)
                {
                    activeScreen = Screen::SystemPerifericsMenu;
                }
                else if (selectedKeypad == KeypadInfoOption)
                {
                    activeScreen = Screen::KeypadInfo;
                }
                else if (selectedKeypad == KeypadTimeoutOption)
                {
                    activeScreen = Screen::KeypadTimeout;
                }
                else if (selectedKeypad == KeypadEventStateOption)
                {
                    activeScreen = Screen::KeypadEventState;
                }
                else if (selectedKeypad == KeypadEventValueOption)
                {
                    activeScreen = Screen::KeypadEventValue;
                }
                else
                {
                    KeypadPeripheral::hardwareReset();
                    lastKeypadResetMs = millis();
                    activeScreen = Screen::KeypadReset;
                }
                break;
            case Screen::SystemSensorsMenu:
                ...
                {
                    activeScreen = Screen::BusUnitsMenu;
                }
                else
                {
                ...
                break;
            case Screen::BusUnitsMenu:
                if (selectedBusUnit == BusUnitBack)
                {
                    activeScreen = Screen::SystemSensorsMenu;
                }
                else if (selectedBusUnit == BusUnitTemperature)
                {
                    activeScreen = Screen::TemperatureUnit;
                }
                else
                {
                    activeScreen = Screen::BusGasUnit;
                }
                break;
            case Screen::SystemNotificationsMenu:
            ...
            case Screen::TemperatureUnit:
                ...
                break;
            case Screen::DisplayDesign:
                DeskSettings::data().displayDesign = (DeskSettings::data().displayDesign + 1) % DISPLAY_HOME_DESIGN_COUNT_DEFAULT;
                DeskSettings::markDirty();
                break;
            case Screen::DisplayTimeoutEnabled:
                DeskSettings::data().displayTimeoutEnabled = DeskSettings::data().displayTimeoutEnabled ? 0 : 1;
                DeskSettings::markDirty();
                break;
            case Screen::DisplayActiveMode:
                DeskSettings::data().displayActiveMode =
                    DeskSettings::data().displayActiveMode >= DISPLAY_ACTIVE_MODE_USB_MOUNTED_DEFAULT ? DISPLAY_ACTIVE_MODE_ALL_TIME_DEFAULT : DeskSettings::data().displayActiveMode + 1;
                DeskSettings::markDirty();
                break;
            case Screen::BusScan:
                scanI2c();
    ...
    void handleTurn(int16_t delta)
    {
        switch (activeScreen)
        {
            ...
            case Screen::SystemMenu:
                ...
                break;
            case Screen::SystemDisplayMenu:
                moveSelection(selectedDisplay, DisplayOptionCount, delta);
                break;
            case Screen::SystemPerifericsMenu:
                moveSelection(selectedPeriferics, PerifericsOptionCount, delta);
                break;
            case Screen::KeypadMenu:
                moveSelection(selectedKeypad, KeypadOptionCount, delta);
                break;
            case Screen::SystemSensorsMenu:
                moveSelection(selectedSystemSensor, SystemSensorOptionCount, delta);
                break;
            case Screen::BusUnitsMenu:
                moveSelection(selectedBusUnit, BusUnitOptionCount, delta);
                break;
            case Screen::SystemNotificationsMenu:
            ...
            case Screen::SystemHomeViewMenu:
                ...
                break;
            case Screen::LightBrightnessValue:
                StripLight::setBrightness(adjustedValue(StripLight::brightness(), delta, 512, STRIP_BRIGHTNESS_MIN_DEFAULT, STRIP_PWM_MAX_DEFAULT));
                StripLight::setEnabled(true);
                markStripConfigChanged();
                break;
            case Screen::LightBrightnessMin:
                StripLight::setBrightnessMin(adjustedValue(StripLight::brightnessMin(), delta, 512, STRIP_BRIGHTNESS_MIN_DEFAULT, STRIP_PWM_MAX_DEFAULT));
                markStripConfigChanged();
                break;
            case Screen::LightBrightnessMax:
                StripLight::setBrightnessMax(adjustedValue(StripLight::brightnessMax(), delta, 512, STRIP_BRIGHTNESS_MIN_DEFAULT, STRIP_PWM_MAX_DEFAULT));
                markStripConfigChanged();
                break;
            case Screen::LightKelvinValue:
                StripLight::setKelvinMix(adjustedValue(StripLight::kelvinMix(), delta, 512, STRIP_HOT_MIN_DEFAULT, STRIP_PWM_MAX_DEFAULT));
                StripLight::setEnabled(true);
                markStripConfigChanged();
                break;
            case Screen::LightKelvinColdMin:
                StripLight::setColdMin(adjustedValue(StripLight::coldMin(), delta, 512, STRIP_COLD_MIN_DEFAULT, STRIP_PWM_MAX_DEFAULT));
                markStripConfigChanged();
                break;
            case Screen::LightKelvinColdMax:
                StripLight::setColdMax(adjustedValue(StripLight::coldMax(), delta, 512, STRIP_COLD_MIN_DEFAULT, STRIP_PWM_MAX_DEFAULT));
                markStripConfigChanged();
                break;
            case Screen::LightKelvinHotMin:
                StripLight::setHotMin(adjustedValue(StripLight::hotMin(), delta, 512, STRIP_HOT_MIN_DEFAULT, STRIP_PWM_MAX_DEFAULT));
                markStripConfigChanged();
                break;
            case Screen::LightKelvinHotMax:
                StripLight::setHotMax(adjustedValue(StripLight::hotMax(), delta, 512, STRIP_HOT_MIN_DEFAULT, STRIP_PWM_MAX_DEFAULT));
                markStripConfigChanged();
                break;
            case Screen::LightButtonClickTime:
            ...
            case Screen::LightButtonDuringTime:
                ...
                break;
            case Screen::LightBrightnessStep:
                StripLight::setBrightnessStep(adjustedValue(StripLight::brightnessStep(), delta, 128, STRIP_STEP_MIN_DEFAULT, STRIP_STEP_MAX_DEFAULT));
                markStripConfigChanged();
                break;
            case Screen::LightKelvinStep:
                StripLight::setKelvinStep(adjustedValue(StripLight::kelvinStep(), delta, 128, STRIP_STEP_MIN_DEFAULT, STRIP_STEP_MAX_DEFAULT));
                markStripConfigChanged();
                break;
            case Screen::DisplayTimeout:
            ...
            case Screen::DisplayDesign:
            {
                ...
                if (design < 0)
                {
                    design = DISPLAY_HOME_DESIGN_COUNT_DEFAULT - 1;
                }
                else if (design >= DISPLAY_HOME_DESIGN_COUNT_DEFAULT)
                {
                    design = 0;
                }
                DeskSettings::data().displayDesign = static_cast<uint8_t>(design);
                DeskSettings::markDirty();
                break;
            }
            case Screen::DisplayTimeoutEnabled:
            ...
            case Screen::DisplayActiveMode:
            {
                int16_t mode = static_cast<int16_t>(DeskSettings::data().displayActiveMode) + (delta > 0 ? 1 : -1);
                if (mode < DISPLAY_ACTIVE_MODE_ALL_TIME_DEFAULT)
                {
                    mode = DISPLAY_ACTIVE_MODE_USB_MOUNTED_DEFAULT;
                }
                else if (mode > DISPLAY_ACTIVE_MODE_USB_MOUNTED_DEFAULT)
                {
                    mode = DISPLAY_ACTIVE_MODE_ALL_TIME_DEFAULT;
                }
                DeskSettings::data().displayActiveMode = static_cast<uint8_t>(mode);
                DeskSettings::markDirty();
                break;
            }
            case Screen::DisplayActiveStart:
                DeskSettings::data().displayActiveStartMinute = adjustedValue(DeskSettings::data().displayActiveStartMinute, delta, DISPLAY_ACTIVE_MINUTE_STEP_DEFAULT, 0, 1439);
                DeskSettings::markDirty();
                break;
            case Screen::DisplayActiveEnd:
                DeskSettings::data().displayActiveEndMinute = adjustedValue(DeskSettings::data().displayActiveEndMinute, delta, DISPLAY_ACTIVE_MINUTE_STEP_DEFAULT, 0, 1439);
                DeskSettings::markDirty();
                break;
            case Screen::SensorSampleInterval:
    ...
    void drawActiveScreen()
    {
        switch (activeScreen)
        {
            ...
            case Screen::SystemDisplayMenu:
                ...
                break;
            case Screen::SystemPerifericsMenu:
                drawOptionList("Periferics", perifericsOptions, PerifericsOptionCount, selectedPeriferics);
                break;
            case Screen::KeypadMenu:
                drawOptionList("Keypad", keypadOptions, KeypadOptionCount, selectedKeypad);
                break;
            case Screen::ChipInfo:
                drawChipInfo();
                break;
            ...
            case Screen::KeypadTimeout:
                ...
                break;
            case Screen::KeypadEventState:
                drawKeypadEventState();
                break;
            case Screen::KeypadEventValue:
                drawKeypadEventValue();
                break;
            case Screen::KeypadReset:
                drawKeypadReset();
                break;
            case Screen::SystemSensorsMenu:
                ...
                break;
            case Screen::BusUnitsMenu:
                drawOptionList("Units", busUnitOptions, BusUnitOptionCount, selectedBusUnit);
                break;
            case Screen::SystemNotificationsMenu:
            ...
            case Screen::LightMode:
                ...
                break;
            case Screen::LightBrightnessValue:
                drawRawEdit("Brightness", StripLight::brightness(), "turn: 0..65535");
                break;
            case Screen::LightBrightnessMin:
                drawRawEdit("Brightness min", StripLight::brightnessMin(), "turn: 0..65535");
                break;
            case Screen::LightBrightnessMax:
                drawRawEdit("Brightness max", StripLight::brightnessMax(), "turn: 0..65535");
                break;
            case Screen::LightKelvinValue:
                drawRawEdit("Kelvin", StripLight::kelvinMix(), "0 warm, 65535 cold");
                break;
            case Screen::LightKelvinColdMin:
            ...
            case Screen::DisplayDesign:
                ...
                break;
            case Screen::DisplayTimeoutEnabled:
                drawDisplayTimeoutEnabled();
                break;
            case Screen::DisplayActiveMode:
                drawDisplayActiveMode();
                break;
            case Screen::DisplayActiveStart:
                drawDisplayActiveTime("Clock start", DeskSettings::dataConst().displayActiveStartMinute);
                break;
            case Screen::DisplayActiveEnd:
                drawDisplayActiveTime("Clock end", DeskSettings::dataConst().displayActiveEndMinute);
                break;
            case Screen::TemperatureUnit:
                drawTemperatureUnitEdit();
                break;
            case Screen::BusGasUnit:
                drawPlaceholder("ppm/%", "Pending gas unit", "Current: ppm");
                break;
            case Screen::SensorSampleInterval:
...
namespace OledPanel
{
    ...
    void update()
    {
        ...
        const bool shouldBeActive = displayShouldBeActive();
        if (displayPowerSave == shouldBeActive)
        {
            displayPowerSave = !shouldBeActive;
            Oled.setPowerSave(displayPowerSave);
            needsRedraw = shouldBeActive;
        }
        ...
        if (DeskSettings::dataConst().displayTimeoutEnabled &&
            activeScreen != Screen::Home &&
            now - lastInteractionMs >= DeskSettings::data().displayTimeoutMs)
        {
