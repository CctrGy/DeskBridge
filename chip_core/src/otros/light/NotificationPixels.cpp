...
namespace
{
    ...
    bool lastSystemBusFault = false;
    bool systemBusFaultStateKnown = false;
...
namespace NotificationPixels
{
    ...
    void update()
    {
        const bool systemBusFault = !DeskBus::systemHealthy();
        if (systemBusFaultStateKnown && systemBusFault == lastSystemBusFault)
        {
            return;
        }

        systemBusFaultStateKnown = true;
        lastSystemBusFault = systemBusFault;

        if (systemBusFault)
        {
            pixels.setPixelColor(NOTIFICATION_SYSTEM_BUS_PIXEL_DEFAULT, pixels.Color(255, 0, 0));
        }
        else
        {
            pixels.setPixelColor(NOTIFICATION_SYSTEM_BUS_PIXEL_DEFAULT, 0);
        }
    ...
    void clear()
    {
        systemBusFaultStateKnown = false;
        pixels.clear();
        pixels.show();
    }
