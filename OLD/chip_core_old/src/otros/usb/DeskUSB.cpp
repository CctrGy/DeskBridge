...
namespace
{
    ...
    void configureUsbFs()
    {
        ...
        GPIO_InitTypeDef usbState = {};
        usbState.Pin = GPIO_PIN_9;
        usbState.Mode = GPIO_MODE_INPUT;
        usbState.Pull = GPIO_PULLDOWN;
        HAL_GPIO_Init(GPIOA, &usbState);

        usbState.Pin = GPIO_PIN_10;
        usbState.Pull = GPIO_PULLUP;
        HAL_GPIO_Init(GPIOA, &usbState);
...
namespace DeskUSB
{
    ...
    bool idGrounded()
    {
        return HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10) == GPIO_PIN_RESET;
    }
